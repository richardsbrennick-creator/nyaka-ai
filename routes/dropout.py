"""
Student Dropout Predictor Routes
"""

from flask import Blueprint, render_template, request, jsonify, flash, redirect, url_for
from flask_login import login_required
from models import Student
from utils.dropout_model import predict_dropout, batch_predict
...
@login_required
def run_all():
    """Re-run predictions on all students and update DB."""
    students = Student.query.all()
    updated = 0
    for s in students:
        data = {
            "attendance_pct":  s.attendance_pct,
            "avg_grade_score": s.avg_grade_score,
            "meals_per_day":   s.meals_per_day,
            "distance_km":     s.distance_km,
            "has_both_parents":s.has_both_parents,
            "caregiver_is_gm": s.caregiver_is_gm,
            "num_siblings":    s.num_siblings,
            "prev_dropout":    s.prev_dropout,
        }
        result = predict_dropout(data)
        ...
        updated += 1
    db.session.commit()
    flash(f"Updated predictions for {updated} students.", "success")
...
def send_alert(student_id):
    """Send SMS dropout alert to grandmother."""
